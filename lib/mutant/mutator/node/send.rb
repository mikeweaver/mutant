    ...
    class Node
      ...
      private
        ...
        def emit_dig_mutation
          return unless selector.equal?(:dig) && arguments.any?

          fetch_mutation = s(:send, receiver, :fetch, arguments.first)

          return emit(fetch_mutation) if arguments.one?

          emit(s(:send, fetch_mutation, :dig, *arguments.drop(1)))
        end
